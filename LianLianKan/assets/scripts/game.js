        ...
        for(var y = 0;y<4;y++){
            for(var x = 0;x < 4;x++){
                ...
                //   self.display.string =      //用于Label显示
                 ...
                 if(self.mcount==this.getComponent('pic').mcount){    //如果位置相等，则是点自己
                    self.mcount=100;
                    self.state=11;
                     cc.audioEngine.playEffect(self.ErrorAudio,false);
                 }
                       else if(self.state==11&&self.mcount==100){     //如果图片和位置都是初始值，则是第一个
                    ...
                    }else if(self.state==this.getComponent('pic').num){         //如果相同图片
                        // this.getComponent(cc.Sprite).spriteFrame=self.y0SpriteFrame;
                     
                        if(self.isLinked(this.getComponent('pic').mcount,t_arrary)){   //并且可连通
